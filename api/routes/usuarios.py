...
def criar():
    ...
    try:
        ...
        from ..validation.input_validator import validar_usuario_input
        erros = validar_usuario_input(dados)
        if erros:
        ...
        if usuario_logado.tipo_usuario != TipoUsuario.RH.value and usuario_logado.flag_gestor != FlagGestor.SIM.value:
        ...
        # Instead of using verificar_email_existente, we'll handle this in the try/catch block
        # Remove these lines:
        # from ..database.crud import verificar_email_existente
        # if verificar_email_existente(dados["email"]):
        #     return jsonify({"erro": "Email já cadastrado"}), 400

        # The email uniqueness will be handled by the IntegrityError catch block below
...
def deletar(cpf: int):
  ...
  try:
      ...
      # Verifica se tem permissão para desativar este usuário específico
      usuario_alvo = get_user(cpf)
      if not usuario_alvo:
          return jsonify({"erro": "Usuário não encontrado"}), 404
          
      # RH só pode desativar usuários da mesma empresa
      if usuario_logado.tipo_usuario == TipoUsuario.RH.value:
          from ..database.crud import obter_grupo
          if usuario_logado.grupo_id and usuario_alvo.grupo_id:
              grupo_rh = obter_grupo(usuario_logado.grupo_id)
              grupo_alvo = obter_grupo(usuario_alvo.grupo_id)
              if not (grupo_rh and grupo_alvo and grupo_rh.cnpj_empresa == grupo_alvo.cnpj_empresa):
                  return jsonify({"erro": "Sem permissão para desativar usuários de outra empresa"}), 403
      
      # Gestor só pode desativar usuários do mesmo grupo
      if usuario_logado.flag_gestor == FlagGestor.SIM.value and usuario_logado.tipo_usuario != TipoUsuario.RH.value:
          if usuario_logado.grupo_id != usuario_alvo.grupo_id:
              return jsonify({"erro": "Sem permissão para desativar usuários de outro grupo"}), 403
